...
public class ReportPo extends JInternalFrame implements ActionListener,ItemListener
{
...
public ReportPo()
 {
	...
	add(p1);
  	...
  	l1=new JLabel("Purchase Order Report");
	...
	add(p3);
	  ...
	  l3=new JLabel("Purchase Order no:");
	l3.setBounds(20,20,150,25);
	l3.setFont(f);
	p3.add(l3);
  

	c4=new JComboBox();
  	c4.setBounds(150,20,80,25);
	c4.setFont(f);
	...
	l5=new JLabel("Total:");
  	l5.setBounds(480,20,80,25);
	l5.setFont(f);
	p3.add(l5);
	
	t2=new JTextField(20);
  	t2.setBounds(550,20,100,25);
	t2.setFont(f);
	...
	void addPOrderDetail()
	{
		try
		{
			String q="Select podate,amount from porder where pno="+c4.getSelectedItem();
			...
			String q2 = "select srno as'Sr no.',description as 'Description',qty as'Quantity',prate as 'Purchase rate',total as 'Total' from po_ri where pno="+c4.getSelectedItem();
	...
	public void actionPerformed(ActionEvent ae)
	{
		...
		void addPOrderno()
		{
			try
			{
				...
				{
					query = "select pno from porder where podate like '"+c1.getSelectedItem()+"-"+c2.getSelectedItem()+"-"+c3.getSelectedItem()+"%' ";
				}
				else if(wise.equals("Month wise"))
				{
					query = "select pno from porder where podate like '"+c1.getSelectedItem()+"-"+c2.getSelectedItem()+"-%' ";
				}
				else if(wise.equals("year wise"))
				{
					query = "select pno from porder where podate like '"+c1.getSelectedItem()+"-%' ";
				}
