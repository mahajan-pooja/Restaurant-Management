...
public class ReportGRN extends JInternalFrame implements ActionListener,ItemListener
{
...
public ReportGRN()
 {
	...
	l6=new JLabel("PO no.");
  	l6.setBounds(670,20,80,25);
	l6.setFont(f);
	p3.add(l6);
	
	t3=new JTextField(20);
  	t3.setBounds(730,20,100,25);
	t3.setFont(f);
	...
	void addGRNDetail()
	{
		try
		{
			String q="Select date,amount,pno from grn1 where grno="+c4.getSelectedItem();
			...
			String q2 = "select srno as'Sr no.',description as 'Description',qty as'Quantity',prate as 'Purchase rate',total as 'Total' from grndesc where grno="+c4.getSelectedItem();
	...
	public void actionPerformed(ActionEvent ae)
	{
		...
		void addGRno()
		{
			try
			{
				...
				{
					query = "select grno from grn1 where date like '"+c1.getSelectedItem()+"-"+c2.getSelectedItem()+"-"+c3.getSelectedItem()+"%' ";
				}
				else if(wise.equals("Month wise"))
				{
					query = "select grno from grn1 where date like '"+c1.getSelectedItem()+"-"+c2.getSelectedItem()+"-%' ";
				}
				else if(wise.equals("year wise"))
				{
					query = "select grno from grn1 where date like '"+c1.getSelectedItem()+"-%' ";
				}
